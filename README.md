...
| **主分支不叫 main** | push 到 develop 等分支时，正式更新静默不触发（PR 预览仍正常） | 检查 workflow 的触发条件——分支**默认只监听 main/master**，要自己改 |

判断标准很简单：如果一个故障不报错，它就必须靠主动巡检发现。

**分支要自己动手配，这是新接手的人最容易漏的一步。** 工具本身不认分支，只认「两个版本之间的
diff」；默认监听哪条分支，完全由 workflow 的触发条件决定。自带的示例按 **`main` / `master`**
写（GitLab 跟随仓库的默认分支设置）。主分支叫别的名字，把 GitHub workflow 里的
`branches: [main, master]` 改成你的分支名即可；想跨分支比对，用
`node scripts/docs-update.mjs --mode preview --base=origin/xxx --head=yyy` 手动指定。
